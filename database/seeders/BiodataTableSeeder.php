<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class BiodataTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DB::table("biodatas")->delete();
        DB::table("biodatas")->insert([
            [
                "nama_lengkap" => "Rehan Maulana",
                "jenis_kelamin" => "Laki-laki",
                "tanggal_lahir" => "2000-01-01",
                "tempat_lahir" => "Jakarta",
                "agama" => "Islam",
                "alamat" => "Jl. Merdeka No. 1, Jakarta",
                "telepon" => "081234567890",
                "email" => "r@gmail.com",
            ],
            [
                "nama_lengkap" => "Siti Aminah",
                "jenis_kelamin" => "Perempuan",
                "tanggal_lahir" => "1995-05-10",
                "tempat_lahir" => "Bandung",
                "agama" => "Islam",
                "alamat" => "Jl. Sudirman No. 2, Bandung",
                "telepon" => "082345678901",
                "email" => "s@gmail.com",
            ],
            [
                "nama_lengkap" => "Budi Santoso",
                "jenis_kelamin" => "Laki-laki",
                "tanggal_lahir" => "1988-12-20",
                "tempat_lahir" => "Surabaya",
                "agama" => "Kristen",
                "alamat" => "Jl. Diponegoro No. 3, Surabaya",
                "telepon" => "083456789012",
                "email" => "b@gmail.com",
            ],
            [
                "nama_lengkap" => "Dewi Lestari",
                "jenis_kelamin" => "Perempuan",
                "tanggal_lahir" => "1992-07-15",
                "tempat_lahir" => "Yogyakarta",
                "agama" => "Hindu",
                "alamat" => "Jl. Malioboro No. 4, Yogyakarta",
                "telepon" => "084567890123",
                "email" => "d@gmail.com",
            ],
            [
                "nama_lengkap" => "Agus Wijaya",
                "jenis_kelamin" => "Laki-laki",
                "tanggal_lahir" => "1985-03-30",
                "tempat_lahir" => "Medan",
                "agama" => "Buddha",
                "alamat" => "Jl. Sisingamangaraja No. 5, Medan",
                "telepon" => "085678901234",
                "email" => "a@gmail.com",
            ],
            [
                "nama_lengkap" => "Linda Putri",
                "jenis_kelamin" => "Perempuan",
                "tanggal_lahir" => "1998-11-25",
                "tempat_lahir" => "Semarang",
                "agama" => "Katolik",
                "alamat" => "Jl. Pandanaran No. 6, Semarang",
                "telepon" => "086789012345",
                "email" => "l@gmail.com",
            ],
            [
                "nama_lengkap" => "Rina Sari",
                "jenis_kelamin" => "Perempuan",
                "tanggal_lahir" => "1990-09-09",
                "tempat_lahir" => "Malang",
                "agama" => "Islam",
                "alamat" => "Jl. Ijen No. 7, Malang",
                "telepon" => "087890123456",
                "email" => "ri@gmail.com"
            ],
            [
                "nama_lengkap" => "Andi Pratama",
                "jenis_kelamin" => "Laki-laki",
                "tanggal_lahir" => "1987-06-18",
                "tempat_lahir" => "Palembang",
                "agama" => "Kristen",
                "alamat" => "Jl. Jendral Sudirman No. 8, Palembang",
                "telepon" => "088901234567",
                "email" => "an@gmail.com"
            ],
            [
                "nama_lengkap" => "Sari Melati",
                "jenis_kelamin" => "Perempuan",
                "tanggal_lahir" => "1993-04-22",
                "tempat_lahir" => "Bogor",
                "agama" => "Hindu",
                "alamat" => "Jl. Pajajaran No. 9, Bogor",
                "telepon" => "089012345678",
                "email" => "sa@gmai;.com"
            ],
            [
                "nama_lengkap" => "Tommy Gunawan",
                "jenis_kelamin" => "Laki-laki",
                "tanggal_lahir" => "1989-02-14",
                "tempat_lahir" => "Depok",
                "agama" => "Buddha",
                "alamat" => "Jl. Margonda No. 10, Depok",
                "telepon" => "080123456789",
                "email" => "t@gmail.com",
            ]
        ]);
    }
}
